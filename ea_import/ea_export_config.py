...
class ea_export_config(orm.Model):
    ...
    _columns = {
        ...
        'model_id': fields.many2one(
            'ir.model', 'Related document model', required=True
        ),
        'ir_act_report_id': fields.many2one(
            'ir.actions.report.xml', 'Report action', readonly=True
        ),
    ...
    def unlink_action(self, cr, uid, ids, context=None):
        for chain in self.browse(cr, uid, ids, context=context):
            if chain.ir_act_report_id:
                self.pool.get('ir.actions.report.xml').unlink(
                    cr, uid, [chain.ir_act_report_id.id], context
                )
    ...
    def generate_csv(self, cr, uid, ids, context={}):
        for config in self.browse(cr, uid, ids, context=context):
            ...
            export_class = CsvExportOpenERPInterface(
                name, config_value, context=context
            )
        output, format = export_class.create(
            cr, uid, ids, {'report_type': u'csv'},
            {'active_ids': [], 'active_model': 'ea_export.config'}
        )
        if output:
            return {
                'type': 'ir.actions.report.xml',
