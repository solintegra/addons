...
class ea_import_template_line(orm.Model):
    ...
    _columns = {
        ...
        'template_id': fields.many2one(
            'ea_import.template', 'Template',
            select=True, ondelete="cascade"
        ),
        'target_model_id': fields.related(
            'template_id', 'target_model_id',
            type='many2one', relation='ir.model', string='Target Object',
        ...
        'related_field_model': fields.char(
            'related_field_model', size=512
        ),
        'sequence': fields.integer(
            'Sequence', required=True,
            select=True
        ),
        'real_field_type': fields.related(
            'target_field', 'ttype', type='char',
            relation='ir.model.fields', string='Field type',
            readonly=True, store=False,
        ),
        ...
        'calc_field_ids': fields.one2many(
            'ea_import.template.line.calc_field',
            'template_line_id', 'Calculated Field'
        ),
        'boolean_field_ids': fields.one2many(
            'ea_import.template.line.boolean_field',
            'template_line_id', 'Boolean Fields'
        ),
        'replace': fields.boolean(
            'Replace field before processing', select=True
        ),
        'regexp_field_ids': fields.one2many(
            'ea_import.template.line.regexp_field',
            'template_line_id', 'Regexp Fields'
        ),
        'header_column_name': fields.char('Header Column Name', size=64,),
        'time_format': fields.char(
            'Time Format', size=128,
            help="""
            DIRECTIVE       MEANING
        ...
        'field_type': fields.selection([
            ...
            ('binary', 'binary'), ('many2one', 'many2one')
        ], 'Field Type'),
        ...
        'match_limit': fields.selection([
            ('match_first', 'Match first result'),
            ('match_last', 'Match last result')],
            'Match Limit',
            help="If defined, will select one record with given "
            "order from several found for template line"
        ),
        'related_template_id': fields.many2one(
            'ea_import.template',
            'Related Template', select=True
        ),
    }

    _defaults = {
        'time_format': '%d/%m/%Y',
        'sequence': lambda self, cr, uid, context: self._get_next_sequence(
    ...
    def name_get(self, cr, uid, ids, context={}):
        result = []
        for template_line in self.browse(cr, uid, ids, context=context):
            template_line_name = "%s/%s" % (
                template_line.target_model_id.name,
                template_line.related_field.name)
            result.append((template_line.id, template_line_name))
        return result

    def default_get(self, cr, uid, fields, context={}):
        result = super(ea_import_template_line, self).default_get(
    ...
    def onchange_target_field(self, cr, uid, ids, field_id, context={}):
        if field_id:
            ...
            field_obj = model_fields_pool.browse(
                cr, uid, field_id, context=context)
            return {'value': {
    ...
    def get_field(
        self, cr, uid, ids, input_list, row_number,
        ...
        def test_many2one_result(result_list, template_line, target_string):
            ...
            if not result_list and template_line.required:
                raise orm.except_orm(
                    _('Error !'),
                    _("""
                    Model - %s\nField - %s\nThere no related field '%s'
                    in %s/%s relation (field is required) \nField name: %s \n
                    Column: %s\nRow number: %i\nRow Data: %s""" %
                        (template_line.template_id.name,
                         ...
                         template_line.target_field.name,
                         str(template_line.sequence),
                         row_number + 1, input_list))
                )

        for template_line in self.browse(cr, uid, ids, context=context):
            if not all([bool(field_str) == False for field_str in input_list]):
                if (
                    (template_line.use_only_defaults or not
                     input_list[template_line.sequence - 1]) and
                    template_line.many2one_rel_type != 'active_id'
                ):
                    ...
                    if (
                        not target_string.strip() and
                        template_line.default_value
                    ):
                        target_string = template_line.default_value
                if template_line.replace:
                    target_string = (
                        template_line_regxp_field_pool.replace_string(
                            cr, uid,
                            [regexp_field.id for regexp_field in
                             template_line.regexp_field_ids],
                            target_string, context=context)
                    )
                template_line_type = template_line.field_type
                if (
                    template_line.calculated and
                    template_line_type in ['integer', 'float']
                ):
                    calc_fields = sorted(
                        template_line.calc_field_ids, key=lambda k: k.sequence)
                    result = float(
                        input_list[
                            calc_fields.pop(0).column_number - 1].strip() or
                        "0")
                ...
                if template_line_type == 'binary':
                    ...
                    if not target_string:
                        raise orm.except_orm(
                            _('Error !'),
                            _("Field %s has no value and cannot be converted. "
                              "Enter a default value in the template for this "
                              "field or fix the CSV file so it contains data "
                              "for this field row. \nField: %s\nColumn: %s\n"
                              "Row number: %i \nRow Data: %s") %
                            (template_line.target_field.name,
                            template_line.target_field.name,
                    ...
                    if (
                        template_line.many2one_rel_type in
                        ['dbid', 'active_id']
                    ):
                        return int(target_string)
                        ...
                        if testing:
                            test_many2one_result(
                                result, template_line, target_string)
                        ...
                        result = (
                            template_line.related_template_id.get_related_id(
                                input_list, row_number, context=context))
                        result = self.match_limit(template_line, result)
                        if testing:
                            test_many2one_result(
                                result, template_line, target_string)
    ...
    def get_selection_from_name(self, cr, uid, ids, target_string, context={}):
        for template_line in self.browse(cr, uid, ids, context=context):
            ...
            selection_dict = target_model_pool._columns[
                template_line.target_field.name].selection
            if isfunction(selection_dict):
                selection_dict = dict(getattr(
                    target_model_pool, selection_dict.__name__)(
                        cr, uid, context=context))
            else:
                selection_dict = dict(selection_dict)
            revrese_selection_dict = (
                dict((v, k) for k, v in selection_dict.iteritems()))
