...
class ea_import_template_unique_rule(orm.Model):
    ...
    _columns = {
        'source_field_id': fields.many2one(
            'ir.model.fields',
            'Template Line Fields', required=True
        ),
        'comparison_model_id': fields.many2one(
            'ir.model',
            'Comparison Model', required=True
        ),
        'target_field_id': fields.many2one(
            'ir.model.fields',
            'Target Model Fields', required=True
        ),
        ...
        'source_model_id': fields.related(
            'template_id', 'target_model_id',
            type='integer', string="Import Template Model", readonly=True, ),
    }
    ...
    def default_get(self, cr, uid, fields, context=None):
        ...
        if context is None:
            context = {}
        result = super(ea_import_template_unique_rule, self).default_get(
    ...
    def onchange_target_field_id(
            self, cr, uid, ids, target_field_id, context={}):
        if target_field_id:
            field_in_table = self.check_field_in_table_presense(
                cr, uid, ids, target_field_id, context=None)
            if not field_in_table.get('field_present'):
                raise orm.except_orm(
                    _('Error !'),
                    _('%s is not storable and can not be selected as target '
                      'field') % (field_in_table.get('target_field_name'))
                )
        return {}
    ...
    def write(self, cr, uid, ids, vals, context=None):
        if vals.get('target_field_id'):
            field_in_table = self.check_field_in_table_presense(
                cr, uid, ids, vals.get('target_field_id'), context=None)
            if not field_in_table.get('field_present'):
                raise orm.except_orm(
                    _('Error !'),
                    _('%s is not storable and can not be selected as target '
    ...
    def check_field_in_table_presense(
            self, cr, uid, ids, target_field_id, context=None):
        res = {}
        if target_field_id:
            target_field_obj = self.pool.get('ir.model.fields').browse(
                cr, uid, target_field_id, context=None)
            ...
            cr.execute(
                ...
                WHERE table_name = '%s'
                AND column_name = '%s' ''' % (
                    comparison_model_table, target_field_name, ))
            result = cr.fetchall()
            res = {
                'field_present': bool(result),
                'target_field_name': target_field_description
            }
        return res


class ea_import_template(orm.Model):
    ...
    _columns = {
        ...
        'create_unique_only': fields.boolean(
            'Create Unique only',
            help="Create record only if one matching key does not already "
            "exist. Do not use with 'update'."
        ),
        'line_ids': fields.one2many(
            'ea_import.template.line',
            'template_id', 'Template Lines',
        ),
        'matching_rules_ids': fields.one2many(
            'ea_import.template.unique.rule',
            'template_id',
            'Import Unique Rule'
        ),
        ...
        'post_import_hook': fields.char(
            'Post-import method', size=512,
            help="Execute method after importing all records.\n"
            "<target_model>.<function_name>(cr, uid, ids_of_imported_records,"
            " context=context)"),
        }
    ...
    def generate_record(
        self, cr, uid, ids, record_list, row_number, context={}
    ):
        context = context or {}
        result = []
        if len(record_list):
            for template in self.browse(cr, uid, ids, context=context):
                ...
                for template_line in template.line_ids:
                    ...
                    value = template_line.get_field(
                        record_list, row_number, testing=True, context=context)
                    if value:
                ...
                if (
                    template.create_new and
                    template.create_unique_only and upd_key
                ):
                    if self.low_level_search(
                        cr, uid, ids, upd_key, context=context
                    ):
                        log_notes = "Record already exists - skipping ", record
                        context['import_log'].append(log_notes)
                        return result
                    # checking if record matching key already exists in fields
                    # defined in matching rules
                    matching_model_field_ids = []
                    for template_line in template.line_ids:
                        field_name = template_line.target_field.name
                        value = template_line.get_field(
                            record_list, row_number,
                            testing=True, context=context)
                        for matching_value in template.matching_rules_ids:
                            if matching_value.source_field_id.name == field_name:
                                matching_model = (
                                    matching_value.comparison_model_id.model)
                                ...
                                matching_model_field_ids = (
                                    self.low_level_search(
                                        cr, uid, ids, matching_model_upd_key,
                    ...
                    if matching_model_field_ids:
                        ...
                        if getattr(new_rec, target_model_pool._rec_name):
                            log_row_name = getattr(
                                new_rec, target_model_pool._rec_name)
                        else:
                            log_row_name = ''
                        log_notes = (
                            "creating ", target_model_pool._name,
                            "- name = ", new_rec._rec_name,
                            "- data = ", record)
                        context['import_log'].append(log_notes)
                ...
                if template.update and upd_key:
                    ...
                    updating_record_id = self.low_level_search(
                        cr, uid, ids, upd_key, context=context)
                    if updating_record_id:
                        ...
                        if self.need_to_update(
                            cr, uid, target_model_pool,
                            updating_record_id, record, context=context
                        ):
                            target_model_pool.write(
                                cr, uid, updating_record_id, record,
                                context=context
                            )
                            result.append(updating_record_id[0])
                            log_notes = ("update ", target_model_pool._name,
                                         "- record id = ", updating_record_id,
                                         "- name = ", log_row_name,
                                         "- data = ", record)
                            context['import_log'].append(log_notes)
                        else:
                            log_notes = ("no update ", target_model_pool._name,
                                         "- record id = ", updating_record_id,
                                         "- name = ", log_row_name,
                                         "- NO CHANGES REQUIRED - data = ",
                                         record)
                            context['import_log'].append(log_notes)
                        updated = True
                if not updated and template.create_new and ready_to_create_new:
                    ...
                    if new_rec._rec_name:
                        ...
                        log_row_name = ''
                    log_notes = ("creating ", target_model_pool._name,
                                 "- name = ", new_rec._rec_name,
                                 "- data = ", record)
                    context['import_log'].append(log_notes)
                    result.append(new_rec_id)
        ...
        for template in self.browse(cr, uid, ids, context=context):
            ...
            for template_line in template.line_ids:
                if template_line.key_field:
                    field_name = template_line.target_field.name
                    value = template_line.get_field(
                        input_list, row_number, testing=True)
                    if value:
                        key.append((field_name, '=', value))
            if template.update_current:
                key.append(('create_date', '>', context['time_of_start']))
            result = self.low_level_search(
                cr, uid, [template.id], key, context=context)
            return result
    ...
    def low_level_search(self, cr, uid, ids, key_list, context={}, **kwargs):
        ...
        for key_sub_list in key_list:
            ...
            where_string += (
                "AND {0} {1} {2} \n".format(
                    key_sub_list[0], key_sub_list[1], second_parametr))
        cr.execute("""
                    SELECT *
                    FROM %s
    ...
    def need_to_update(
        self, cr, uid, target_model_pool,
        updating_record_id, record, context={}
    ):
        for old_record in target_model_pool.read(
            cr, uid, updating_record_id, context=context
        ):
            filtered_old_record = {}
        ...
        return any([filtered_old_record.get(key) != (
            ...
            cr.execute("""UPDATE ea_import_template tmpl
                        ...
                        FROM (SELECT eit.id,
                            ...
                            eicl.post_import_hook
                              ...
                              LEFT JOIN ea_import_chain_link eicl
                              ON eicl.template_id = eit.id) as q1
                        WHERE tmpl.id = q1.id
            """)
        return True
